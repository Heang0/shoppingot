...
export default function AdminSettings() {
  ...
  const handleTelegramResponse = async (response: any) => {
    try {
      setLoading(true);
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:5000'}/api/auth/telegram/link`, {
        method: 'PUT',
        headers: {
  ...
  const handleStoreSubmit = async (e: React.FormEvent) => {
    ...
    setSuccessMsg('');
    try {
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:5000'}/api/stores/${storeData._id}`, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
          Authorization: `Bearer ${user?.token}`
      ...
      if (res.ok) {
        ...
        await fetch(`${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:5000'}/api/stores/${storeData._id}/payment-settings`, {
          method: 'PUT',
          headers: {
            'Content-Type': 'application/json',
            Authorization: `Bearer ${user?.token}`
  ...
  const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>, type: 'profile' | 'storeLogo' | 'banner') => {
    ...
    try {
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:5000'}/api/upload?type=${type}`, {
        method: 'POST',
        headers: { Authorization: `Bearer ${user?.token}` },
        body: uploadData
      });
      ...
      if (res.ok) {
        if (type === 'profile') {
          ...
          await fetch(`${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:5000'}/api/stores/${storeData._id}`, {
            method: 'PUT',

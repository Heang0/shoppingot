...
// --- Product Card ---
const ProductCard = ({ product, primaryColor, onAddToCart }: {
  product: any;
  primaryColor: string;
  ...
  return (
    <Link href={`/${params.locale}/product/${product.slug || product._id}`} className="flex flex-col group h-full">
      <div className="aspect-square w-full relative shrink-0 overflow-hidden mb-3 bg-gray-50 dark:bg-gray-900">
        ...
        <button
          onClick={handleAdd}
          className="absolute bottom-2 right-2 w-8 h-8 flex items-center justify-center text-white rounded-full shadow-lg opacity-0 group-hover:opacity-100 translate-y-1 group-hover:translate-y-0 transition-all duration-200 active:scale-90 md:flex hidden"
      ...
      <div className="flex flex-col flex-1 px-1">
        ...
        <div className="mt-auto pt-2 flex items-center justify-between">
          <span className="text-[13px] font-semibold text-gray-900 dark:text-white">${product.price.toFixed(2)}</span>
          {/* Mobile add button (always visible) */}
          <button
            onClick={handleAdd}
            className="md:hidden w-7 h-7 flex items-center justify-center text-white rounded-full active:scale-90 shadow-sm"
            style={{ backgroundColor: primaryColor || '#000' }}
          >
...
export default function StorefrontHome({ params }: { params: { slug: string } }) {
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 py-6 space-y-8">
      ...
      {/* Banner */}
      {bannerUrl ? (
        <div className="w-full aspect-[21/9] sm:aspect-[3/1] overflow-hidden rounded-2xl bg-gray-100 dark:bg-gray-900">
          {/* eslint-disable-next-line @next/next/no-img-element */}
      ...
      {categories.length > 0 && (
        <div className="flex gap-5 overflow-x-auto pb-1 scrollbar-hide -mx-4 px-4">
          <button
            onClick={() => setActiveCategory('All')}
            className={`whitespace-nowrap text-sm pb-1 border-b-2 transition-all ${
              activeCategory === 'All'
                ? 'font-semibold text-gray-900 dark:text-white border-gray-900 dark:border-white'
                : 'font-medium text-gray-400 border-transparent hover:text-gray-700 dark:hover:text-gray-300'
            }`}
          >
            All ({products.length})
          </button>

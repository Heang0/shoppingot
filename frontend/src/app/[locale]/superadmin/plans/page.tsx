...
export default function PlansManagement() {
  ...
  const fetchPlans = async () => {
    try {
      const res = await fetch('http://192.168.1.7:5000/api/superadmin/plans', {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const url = editingPlan 
        ? `http://192.168.1.7:5000/api/superadmin/plans/${editingPlan._id}`
        : 'http://192.168.1.7:5000/api/superadmin/plans';
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      const res = await fetch(`http://192.168.1.7:5000/api/superadmin/plans/${id}`, {
        method: 'DELETE',
        headers: { Authorization: `Bearer ${user?.token}` }
      });
      if (res.ok) {
